...
export class CanvasComponent implements OnInit {
  ...
  drop(event: CdkDragDrop<string[]>) {
    if (event !== null)
      if (event.container === event.previousContainer) {
        ...
        control.insert(event.currentIndex, this.formGroup(event.container.data[event.currentIndex]));
        ...
        const control = <FormArray> this.formObj.controls['fields'];
        control.insert(event.currentIndex, this.formGroup(event.previousContainer.data[event.previousIndex]))
        console.log("formbuilder????", this.canvasItems, this.formObj.value)
      }
  }
  ...
  updateFormObj(e) {
    ...
    control.insert(this.controllerObj.index, this.formGroup(e));
